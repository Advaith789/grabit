...
class FoodItem(BaseModel):
    ...
    quantity: Optional[str] = Field(description="The quantity of the food. Null if unknown.")

class ExtractionResult(BaseModel):
    foods: list[FoodItem] = Field(description="List of extracted food items")
    email_body: str = Field(description="A friendly, concise email message for the user.")
    email_subject: str = Field(description="An exciting, short subject line for the email.")
...
def process_new_log(restaurant_name: str, message: str):
    ...
    try:
        ...
        system_prompt = (
            ...
            "STRICT RULE: Do not use any emojis in the content. "
            "Respond ONLY with a valid JSON object matching this structure: "
            "{"
            "'foods': [{'name': 'item', 'cuisine': 'type', 'quantity': 'number'}], "
