...
def restaurant_signup(restaurant: RestaurantCreate, db: Session = Depends(get_db)):
    db_restaurant = (
        ...
        .filter(DBRestaurant.restaurant_email == restaurant.restaurant_email)
        .first()
    )
...
def restaurant_echo(payload: TextPayload, db: Session = Depends(get_db)):
    db_restaurant = (
        db.query(DBRestaurant)
        .filter(DBRestaurant.restaurant_email == payload.restaurant_email)
        .first()
    )
...
def get_users_by_restaurant(
    payload: RestaurantSearchPayload, db: Session = Depends(get_db)
):
    # Returns the user_email for users whose JSONB array overlaps with the search payload
    results = (
        db.query(DBUser.user_email)
        .filter(DBUser.restaurants.op("?|")(array(payload.restaurants)))
        .all()
    )
